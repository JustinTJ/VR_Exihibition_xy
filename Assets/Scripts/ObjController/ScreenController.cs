...
public class CubeController : MonoBehaviour
{
    ...
    void ResetSlidersToDefault()
    {
        ...
        posYSlider.value = 0;  // Default position Y
        ...
        // scaleZ is fixed at 0.05f in UpdateScale, so no need to reset it
    }
    ...
    void UpdateScale(float value)
    {
        if (cube != null)
        {
            cube.transform.localScale = new Vector3(scaleXSlider.value, scaleYSlider.value, 0.05f); // Z scale is fixed at 0.05f
        }
    }
}
